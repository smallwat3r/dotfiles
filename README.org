...
- GPD Pocket 3 (running on Arch) specific configurations can be found in ~gpd-arch~ and ~gpd-arch-root~. It includes stowing the generic ~arch~ and ~arch-root~ configs, but adds various tweaks to make the GPD running correctly on Arch Linux (screen rotation and resolution, touchscreen support, X11 configs etc).
